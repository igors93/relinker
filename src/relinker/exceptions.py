"""
Custom exceptions used by Relinker.
"""
...
class RelinkerError(Exception):
    """Base exception for all errors raised by Relinker itself."""


class InvalidRetryConfigError(RelinkerError, ValueError):
    """
    Raised when a retry policy receives an impossible configuration.

    Relinker should not block user decisions, but it should reject values that
    cannot make sense, such as negative attempts or negative delays.
    """


class RetryExhaustedError(RelinkerError):
    """
    Raised when retry attempts are exhausted and the user chooses library-level failure.

    By default, Relinker re-raises the last original exception for exception-based
    failures and returns the last value for result-based retries. This exception is
    available for users who prefer a Relinker-specific failure object.
    """
